...
void squareRoot(int hugeInt[], int sqrt[], int hugeIntSize, int& sqrtSize)
{
	...
	int product[arraySize], productSize = hugeIntSize; // add
	...
	for (int i = sqrtSize - 1; i >= 0 ; i--)	
	{ //add
		for(; !less(hugeInt, product, hugeIntSize, productSize);){ //add
			sqrt[i]++; //add
			square(sqrt, product, sqrtSize, productSize); //add
		} //add
		sqrt[i]--; //add
	}

	//for (int i = hugeIntSize; i >= 0; i--)		//delete
		//{		//delete
		//if (hugeInt[i] == product[i])		//delete
			//sqrt[sqrtSize-1]++;		//delete
		//else if (hugeInt[i] > product[i])		//delete
